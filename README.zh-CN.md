...
- **及时获知变化**：定时快照 + 基于阈值的**邮件预警**，邮件正文由 AI 自动撰写，并附带 Excel 数据文件。
...
| 🔔 | **邮件预警** | 为指标设置阈值规则；触发时按你的定时计划发送一封 **AI 撰写**的邮件，并将指标数据作为 **Excel** 附件携带。 |
| 🧠 | **知识库** | 从对话中沉淀业务知识，为后续回答提供依据。 |
| 🌍 | **国际化** | 内置中英文界面。 |
...
```
   ...
   (平台元数据)                    (MySQL · PG · Oracle)              (OpenAI 兼容)
            ...
            │                                                                  
   后台调度器 ─────────────────────▶ SMTP 服务器
   (快照 · 邮件预警)                  (预警邮件 + Excel 附件)
```

除元数据库外，API 服务本身是无状态的；SPA 通过同源的 `/api` 前缀与之通信（包含位于 `/api/chat` 的 WebSocket）。服务进程内运行两个后台调度器 —— 一个负责指标**快照**，一个负责**邮件预警** —— 两者都以原子方式领取到期任务，因此可安全地多实例运行。
...
> 大模型服务、API Key、模型名以及 SMTP 设置均在应用内运行时配置（设置 / 邮件预警），保存在数据库中 —— 无需环境变量。
...
- 🙈 **凭据处理** —— 数据源/大模型/SMTP 的凭据绝不会被 API 返回。
